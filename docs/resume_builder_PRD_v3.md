...
The LLM must not invent or generate: name, phone number, address, or email. These are pulled from the user's account and injected into the final assembled resume at assembly time.

### 5.3 Resume generation is section-based
The system generates resume sections independently so users can later regenerate individual sections without affecting the rest of the resume.
...
Generation runs section by section via LangChain calling OpenRouter.
...
- Each section is a separate LLM call so it can be independently regenerated
...
After all sections are generated, a separate LangChain/OpenRouter call runs a validator over the combined output before assembly.

**Validator must check for:**
...
- **Auto-correct** — small formatting or structural issues fixed in place; proceed to assembly
...
3. System sends: current section content + base resume section + job description + user instructions + aggressiveness level to LLM via LangChain/OpenRouter
4. Validator reviews the regenerated section output
...
4. Full generation pipeline reruns (§10.7 → §10.8 → §10.9)
